...
fun ChatScreen(currentUser: User, recipientUser: User, onSearchUsers: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        Firebase.database.reference.child("messages").child(conversationId)
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Row(modifier = Modifier.padding(16.dp)) {
            ...
            Button(onClick = {
                ...
                Firebase.database.reference.child("messages").child(conversationId).push().setValue(message)
                messageText = ""
            }) {
                Text("Send")
            }
        }
        Button(onClick = onSearchUsers, modifier = Modifier.padding(16.dp)) {
            Text("Search Users")
        }
    }
}
...
fun MessageCard(message: Message) {
    Card(
        ...
        Column(modifier = Modifier.padding(8.dp)) {
            ...
            Text(text = message.text, style = MaterialTheme.typography.displayMedium)
        }
    }
}
